...
export function driversForScope(scope, data = loadData()) {
  ...
  const attachCounts = (d) => {
    ...
    return {
      ...
      rtsEventCount: ev.rts.filter(m).length,
    };
  };

  if (scope !== "all") {
    return driversForWeek(scope, data).map(attachCounts);
  }
  ...
  return [...byId.values()].map((g) => {
    const recs = g.recs;
    const overall = wavg(recs, "overall");
    const agg = {
      id: g.id,
      name: g.name,
      week: "all",
      ...
      overall,
      tierText: overall !== null ? scoreTier(overall).label : null,
