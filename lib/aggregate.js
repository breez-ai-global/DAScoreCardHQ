...
import { safetyForName } from "./safety";

export { weekOfDate, weekWindow, weekTitle };

// How many of a driver's scoped RTS events actually hurt the scorecard?
// Only returns that impacted DCR (Impact DCR = Y) AND had no exemption applied
// count as a penalty — an exempted or non-impacting return did not ding us, so
// it must not deduct from the driver's Breez score.
function countControllableRts(rtsEvents) {
  return rtsEvents.filter((e) => {
...
// Folds in the Netradyne driving-safety overlay (matched by driver name).
function applyScore(d) {
  const amazon = d.overall ?? null;
  const safety = safetyForName(d.name);
  d.safety = safety;
  const s = breezScore({
...
export function buildOverview(scope, data = loadData()) {
  ...
  const attention = ranked.filter(
    (d) =>
      ...
      d.feedbackCount > 0 ||
      d.concessionCount > 0 ||
      (d.scoreParts && d.scoreParts.safetyPts >= 4)
  );
  ...
  return {
    ...
    ranked: ranked.map((d) => ({
      ...
      safety: d.safety ?? null, safetyPts: d.scoreParts?.safetyPts ?? 0,
