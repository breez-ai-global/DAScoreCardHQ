...
export function overview(data = loadData()) {
  ...
  // tier mix on the Breez scale: Platinum 98+ · Gold 90+ · Silver 80+ · Bronze 70+ · At Risk <70
...
// Pass evOverride (a {feedback,concessions,rts} set already scoped to a week) to
// scope the insight sections; otherwise all events for this driver are used.
export function driverInsights(d, data = loadData(), evOverride = null) {
  const ev = evOverride || driverEvents(d.id, d.name, data);
  const severe = [];
  const improve = [];
  const grow = [];
  const well = [];
  ...
  if (ev.feedback.length) {
    severe.push({
      title: `${ev.feedback.length} customer complaint(s) in view:`,
  ...
  if (!ev.feedback.length) well.push({ title: "Zero customer complaints in view." });
