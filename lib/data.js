...
// Honest tier: Amazon may grade a driver Gold while their overall score is in
// the red. Anyone under 70 is shown as "At Risk" regardless of Amazon's label.
export function effectiveTier(d) {
  if (d && d.overall !== null && d.overall !== undefined && d.overall < 70) {
    return { label: "At Risk", cls: "bronze", amazon: d.tierText || null };
  }
  return { label: d?.tierText || null, cls: tierClass(d?.tierText), amazon: null };
}
