      ...
      <div>
        <div className="score-row">
          ...
          <span className="muted" style={{ fontSize: 12 }}>
            {d.weeksActive > 1 ? `avg · ${d.weeksActive} wks` : "weekly score"}
          </span>
        </div>
  ...
  return (
    <>
      ...
      <p className="page-sub">
        {sorted.length} drivers · {scopeName} · tap a card for the full deep-dive
      </p>
      ...
      {watch.length > 0 && (
        <>
          ...
          <div className="driver-grid">
            {watch.map((d) => <DriverCard key={d.id} d={d} rank={sorted.indexOf(d) + 1} />)}
          </div>
        </>
      )}
      ...
      <div className="driver-grid">
        {solid.map((d) => <DriverCard key={d.id} d={d} rank={sorted.indexOf(d) + 1} />)}
      </div>
    </>
  );
}
